...
var onShowGames = function onShowGames(event) {
  event.preventDefault();
  api.showGame().then(ui.showSuccess).catch(ui.showFailure);
};
...
var startCount = function startCount(num) {
  if (num === 0) {
...
var turnValue = function turnValue() {
  if (turnNumber % 2 === 1) {
...
var checkForMatch = function checkForMatch() {
  var winCombo = [[store.game.cells[0], store.game.cells[1], store.game.cells[2]], [store.game.cells[3], store.game.cells[4], store.game.cells[5]], [store.game.cells[6], store.game.cells[7], store.game.cells[8]], [store.game.cells[0], store.game.cells[3], store.game.cells[6]], [store.game.cells[1], store.game.cells[4], store.game.cells[7]], [store.game.cells[2], store.game.cells[5], store.game.cells[8]], [store.game.cells[0], store.game.cells[4], store.game.cells[8]], [store.game.cells[2], store.game.cells[4], store.game.cells[6]]];

  var win = function win() {
    if (winCombo.some(function (key) {
      return key.join('') === 'xxx';
    })) {
      ...
      return true;
    } else if (winCombo.some(function (key) {
      return key.join('') === 'ooo';
    })) {
...
var gameLogic = function gameLogic(num) {
  var game = function game(event) {
    event.preventDefault();
    var obj = { game: { cell: { index: num, value: '' }, over: false } };
    if ($('#' + num).text() === '') {
      $('#' + num).text(turnValue());
      $('#message').text('');
